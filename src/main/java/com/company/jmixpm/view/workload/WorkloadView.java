...
public class WorkloadView extends StandardView {
    ...
    @ViewComponent
    private DataGrid<Object> workloadDataGrid;
    @Autowired
    ...
    public void onWorkloadDataGridWorkloadInfo(final ActionPerformedEvent event) {
        KeyValueEntity item = (KeyValueEntity) workloadDataGrid.getSingleSelectedItem();
        if (item == null) {
            return;
        }

        dialogWindows.view(this, WorkloadInfoView.class)
                .withViewConfigurer(workloadInfoView -> workloadInfoView.withItem(item))
                .open();
