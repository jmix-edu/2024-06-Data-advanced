...
public interface DeveloperRole {
    ...
    @MenuPolicy(menuIds = {"Project.list", "Task_.list", "TimeEntry.list", "User.list", "PersonalData.list"})
    @ViewPolicy(viewIds = {"Project.list", "Task_.list", "TimeEntry.list", "Project.detail", "Task_.detail", "TimeEntry.detail", "User.list", "User.detail", "PersonalData.list", "PersonalData.detail"})
    void screens();

    @EntityAttributePolicy(entityClass = Project.class, attributes = "tasks", action = EntityAttributePolicyAction.MODIFY)
    @EntityAttributePolicy(entityClass = Project.class, attributes = {"id", "totalEstimatedEfforts", "name", "startDate", "endDate", "manager", "status", "participants", "roadmap", "projectLabels", "deletedBy", "deletedDate", "ownerId", "owner"}, action = EntityAttributePolicyAction.VIEW)
    ...
    @EntityAttributePolicy(entityClass = PersonalData.class, attributes = "*", action = EntityAttributePolicyAction.VIEW)
    @EntityPolicy(entityClass = PersonalData.class, actions = EntityPolicyAction.READ)
    void personalData();
}